...
export default async function ClientConversationPage({
  ...
  async function sendClientMessageAction(formData: FormData) {
    ...
    if (!content) {
      redirect(`/client/messages/${conversationId}`);
    }
    ...
    await prisma.notification.create({
      data: {
        userId: validConversation.professional.user.id,
        type: "MESSAGE_RECEIVED",
        title: "Nuevo mensaje",
        content: "Tenés un nuevo mensaje de un cliente.",
        actionUrl: `/professional/messages/${validConversation.id}`,
        entityType: "CONVERSATION",
        entityId: validConversation.id,
      },
    });

    redirect(`/client/messages/${validConversation.id}`);
  }

  const professionalName =
    conversation.professional.user.name ?? conversation.professional.user.email;
    ...
    <main className="min-h-screen bg-slate-100 pb-24 text-slate-950 md:pb-0">
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-slate-200 bg-white/95 px-3 py-2 shadow-2xl backdrop-blur md:hidden">
        <div className="grid grid-cols-4 gap-1">
          ...
          <MobileNavItem href="/client/notifications" label="Avisos" icon={<Bell size={20} />} />
        </div>
      </nav>
    </main>
  );
